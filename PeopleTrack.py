...
out = cv2.VideoWriter('PeopleTrack.avi', fourcc, 20.0, (800, 600))
...
while cap.isOpened():
    ...
    for (x, y, w, h) in people:
        ...
        if is_close(cx, cy, people):
            footprint.append((cx, cy, 0))
        else:
            footprint.append((cx, cy, 1))
    ...
    # cv2.putText(img, "People Counted: {}".format(pc), (10, 30), cv2.FONT_HERSHEY_SIMPLEX,
    # 1, (255, 0, 0), 3)
    l = len(footprintM)
    if l > 254:
        footprintM.pop(0)
    print(l)
    cr = 1
    for fp in footprintM:
        cr += 1
        r = int((cr / l) * 5)
        for (cx, cy, sd) in fp:
            cv2.circle(img, (cx, cy), r, (0, 255, 255-cr), -1)
        '''for (cx, cy, sd) in fp:
            if sd == 0:
